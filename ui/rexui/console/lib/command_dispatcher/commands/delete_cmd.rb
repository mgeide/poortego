...
module PoortegoCommands
  ...
  def cmd_poortego_delete(*args)
    ...
    when 'section'
      if (name =~ /^\d+$/)
        ...
        obj = PoortegoSection.delete_from_name(@poortego_session.session_values["Current Project"].id, name)
      end
    when 'transform'
      if (name =~ /^\d+$/)
    ...
    when 'entity'
      if (name =~ /^\d+$/)
        ...
        obj = PoortegoEntity.delete_from_name(@poortego_session.session_values["Current Project"].id, 
    ...
    when 'link'
      if (name =~ /^\d+$/)
        ...
        obj = PoortegoLink.delete_from_name(@poortego_session.session_values["Current Project"].id, 
                                    @poortego_session.session_values["Current Section"].id, 
                                    name)
      end
    when 'descriptor'
      if (name =~ /^\d+$/)
        ...
        obj = PoortegoSectionDescriptor.delete_from_name(@poortego_session.session_values["Current Section"].id, 
                                                 name)
      end
    when 'field'
      if (@poortego_session.session_values["Current Selection Type"] == 'entity')
        if (name =~ /^\d$/)
          ...
          obj = PoortegoEntityField.delete_from_name(@poortego_session.session_values["Current Entity"].id,
                                             name)
        end
        ...
        if (name =~ /^\d$/)
          ...
          obj = PoortegoLinkField.delete_from_name(@poortego_session.session_values["Current Link"].id,
                                           name)
        end
      elsif (@poortego_session.session_values["Current Selection Type"] == 'link_type')
        # Remove field from link type
        obj = PoortegoLinkTypeField.delete_from_name(@poortego_session.session_values["Current LinkType"].id,
                                             name)
      elsif (@poortego_session.session_values["Current Selection Type"] == 'entity_type')
        # Remove field from entity type
        obj = PoortegoEntityTypeField.delete_from_name(@poortego_session.session_values["Current EntityType"].id,
                                             name)  
      else
  ...
  # "Delete" command help
  #
  def cmd_poortego_delete_help(*args)
