...
module Commands
  ...
  def cmd_show(*args)
    ...
    if (args.length > 1)
      ...
      when 'section'
        show_obj = PoortegoSection.select(driver.interface.working_values["Current Project"].id, 
                                  show_name)
      when 'transform'
        show_obj = PoortegoTransform.select(show_name)
      when 'entity'
        show_obj = PoortegoEntity.select(driver.interface.working_values["Current Project"].id, 
                                 driver.interface.working_values["Current Section"].id, 
                                 show_name)
      when 'link'
        show_obj = PoortegoLink.select(driver.interface.working_values["Current Project"].id, 
                               driver.interface.working_values["Current Section"].id, 
                               show_name)
      when 'entity_type'
