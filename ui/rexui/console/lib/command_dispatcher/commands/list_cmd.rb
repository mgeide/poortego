...
module PoortegoCommands
  ...
  def cmd_poortego_list(*args)
    ...
    when 'entity', 'entities'          # List Entity                               
      list_objs = PoortegoEntity.list( @poortego_session.session_values["Current Project"].id,
    ...
    when 'link', 'links'              # List Links
      unless ( @poortego_session.session_values["Current Entity"].nil? )
        list_objs =  PoortegoLink.list(@poortego_session.session_values["Current Project"].id, 
                               @poortego_session.session_values["Current Section"].id,
                               @poortego_session.session_values["Current Entity"].id)
      else
        list_objs =  PoortegoLink.list(@poortego_session.session_values["Current Project"].id, 
                               @poortego_session.session_values["Current Section"].id)
      end
