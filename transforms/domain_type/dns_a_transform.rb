...
if (transform.transformInput.include?("entityType"))
  if (transform.transformInput["entityType"] == "domain")
    ...
    resources = dns_resolver.getresources("#{entityValue}", Resolv::DNS::Resource::IN::A)
    resources.each do |resource|
    
      a_result = resource.address.to_s
      
      entityB_attributeHash = Hash.new()
      entityB_attributeHash['title'] = a_result
      entityB_attributeHash['type'] = 'ip_address'
      entityB_fieldHash = Hash.new()
      entityB_fieldHash['source'] = 'Dig_A_Transform'
      ...
      link_attributeHash['entity_b'] = a_result
      link_fieldHash = Hash.new()
      link_fieldHash['source'] = 'Dig_A_Transform'
  ...
  transform.addMessage("DigA Transform", "NOTE", "Entity type not set (#{transform.transformInput['entityType']}), this transform has no action to perform.")
